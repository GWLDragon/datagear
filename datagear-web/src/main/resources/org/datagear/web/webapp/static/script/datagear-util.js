...
(function ($, undefined) {
  $.extend({
            ...
            options,
            {
              ...
              success: function (data, textStatus, jqXHR) {
                ...
                var $dialog = $(
                    "<div id='dialog-" +
                    new Date().getTime() +
                    "' class='dialog-content-container'></div>"
                ).appendTo(container);

                if (options.pageParam) $.pageParam($dialog, options.pageParam);

                if (options.asDialog) $._dialog($dialog, options);
                ...
                if (successCallback) successCallback(data, textStatus, jqXHR);
              },
              type: "POST",
            }
        );
    ...
    pageParamCall: function ($dom, functionName, argArray) {
      ...
      //页面参数是函数
      if ($.isFunction(pageParam)) return pageParam.apply(window, arguments[1]);
    ...
    _dialog: function (element, options) {
      options = $.extend(
          {
            ...
            close: function (event) {
              if (!event.isHideDialog) $(this).dialog("destroy").remove();
            },
      ...
      if (
          ...
          options.height.charAt(options.height.length - 1) == "%"
      )
        options.height =
            (parseInt(options.height.substr(0, options.height.length - 1)) *
                $(window).height()) /
            100;
    ...
    isInDialog: function (dom) {
      var $dialogFlag = $(dom).closest(
          ".dialog-content-container.ui-dialog-content"
      );
    ...
    confirm: function (content, options) {
      options = options || {};
      options = $.extend(
          { confirmText: "Confirm", cancelText: "Cancel", title: "Confirm" },
          options
      );
    ...
    tipSuccess: function (content, delayMs) {
      content =
          "<span class='tooltip-icon ui-icon ui-icon-circle-check'></span>" +
    ...
    tipError: function (content, delayMs) {
      content =
          "<span class='tooltip-icon ui-icon ui-icon-alert'></span>" +
          "<div class='content-value'>" +
          content +
          "</div>";
    ...
    tipInfo: function (content, delayMs) {
      content =
          "<span class='tooltip-icon ui-icon ui-icon-info'></span>" +
          "<div class='content-value'>" +
          content +
          "</div>";
    ...
    _tip: function (tooltipClass, content, delayMs) {
      ...
      var tooltip = $(".global-tooltip", document.body);
      if (tooltip.length > 0) tooltip.tooltip("destroy").remove();
      ...
      tooltip = $("<div class='global-tooltip' style='display:none;' title=''>")
          .attr("id", tooltipId)
          .prependTo(tooltipParent)
          .tooltip({
            ...
            open: function (event, ui) {
              if (delayMs >= 0) {
                var tooltipTimeoutCloseId = window.setTimeout(function () {
                  $.closeTip();
                }, delayMs);
    ...
    _tooltipTimeoutCloseId: function (id) {
      if (id) $(document.body).attr("tooltip-timeout-close-id", id);
      else {
    ...
    formToJson: function (form, ignores) {
      ...
      $(array).each(function () {
        ...
        for (var i = 0; i < names.length; i++) {
          ...
          if (isEleKey) name = name.substring(1, name.length - 1);

          if (i == names.length - 1) {
            //数组元素
            if (isEleKey) {
              if (parent[KeyForArray] == undefined) parent[KeyForArray] = true;

              parent[name] = value;
            } else {
              var preValue = parent[name];

              if (preValue == undefined) parent[name] = value;
              else {
                //末尾允许重名并设为数组，以支持同名表单项
                if ($.isArray(preValue)) preValue.push(value);
                else {
                  parent[name] = [];
            ...
            if (!myParent) myParent = parent[name] = {};
            parent = myParent;
          }
        }
      });
    ...
    splitPropertyPath: function (str) {
      ...
      for (var i = 0; i < str.length; i++) {
        ...
        if (c == "\\") {
          if (i + 1 < str.length) ele += str.charAt(i + 1);
          i += 1;
        } else if (c == ".") {
    ...
    _convertKeyForArrayObj(obj, keyForArray) {
      ...
      for (var p in obj) {
        if (p == keyForArray) continue;

        if (isArrayObj) ps.push(p);

        obj[p] = $._convertKeyForArrayObj(obj[p], keyForArray);
      }

      if (!isArrayObj) return obj;
      ...
      for (var i = 0; i < ps.length; i++) re.push(obj[ps[i]]);
    ...
    copyObject: function (obj, names) {
      ...
      if (names) names = $.isArray(names) ? names : [names];

      var re = [];
      for (var i = 0; i < array.length; i++) {
        re[i] = {};
        for (var p in array[i]) {
          if (!names || $.inArray(p, names) >= 0) re[i][p] = array[i][p];
        }
      }
    ...
    propertyValue: function (obj, name) {
      ...
      var re = [];
      for (var i = 0; i < array.length; i++) re[i] = array[i][name];
    ...
    getPropertyParamString: function (objOrArray, propertyName, paramName) {
      ...
      if (!$.isArray(objOrArray)) objOrArray = [objOrArray];
    ...
    getPropertyParamObjArray: function (objOrArray, propertyName, paramName) {
      if (!paramName) paramName = propertyName;

      var re = [];

      var isArray = $.isArray(objOrArray);

      if (!isArray) objOrArray = [objOrArray];
    ...
    buildDataTablesColumnTitleSearchable: function (titleName) {
      return (
          "<a class='keyword-search-column'>" + $.escapeHtml(titleName) + "</a>"
      );
    },
    ...
    buildDataTablesColumns: function (table, options) {
      options = $.extend(
          { stringDisplayThreshold: 47, keywordQueryColumnCount: -1 },
          options
      );
      ...
      for (var i = 0; i < columns.length; i++) {
        ...
        var isKeywordSearchCol =
            i < options.keywordQueryColumnCount &&
            $.meta.isKeywordSearchColumn(column);

        dtColumns.push({
          title: $.meta.displayInfoHtml(
              column,
              "a",
              isKeywordSearchCol ? "keyword-search-column" : ""
          ),
          ...
          render: function (data, type, row, meta) {
            ...
            renderValue = $.truncateIf(
                renderValue,
                "...",
                _this.options.stringDisplayThreshold
            );
            renderValue = $.escapeHtml(renderValue);

            //解决当所有属性值都为null时，行渲染会很细问题
            if (columnIndex == 0 && renderValue == "") renderValue = " ";

            if (_this.options.postRender) {
              var rowIndex = meta.row;
              if (rowIndex.length) rowIndex = rowIndex[0];

              return _this.options.postRender(
                  data,
                  type,
                  row,
                  meta,
    ...
    getDataTableColumnCount: function (dataTable, row) {
      if (row == undefined) row = dataTable.row(0);
    ...
    getDataTableRowCount: function (dataTable, column) {
      if (column == undefined) column = dataTable.column(0);
    ...
    getDataTableColumnNameCellIndexes: function (settings, cellIndexes) {
      ...
      for (var i = 0; i < cellIndexes.length; i++) {
        ...
        var columnName =
            columnInfos[index.column].columnName ||
            columnInfos[index.column].data;
    ...
    getDataTableRowIndexesMap: function (cellIndexes) {
      ...
      for (var i = 0; i < cellIndexes.length; i++) {
        ...
        var rowIndexes =
            rowIndexesMap[index.row] || (rowIndexesMap[index.row] = []);
    ...
    getDataTableColumnInfos: function (settings) {
      ...
      //column.render函数中的结构
      if (settings.aoColumns) columnInfos = settings.aoColumns;
      //.DataTable().settings()结构
      else if (settings[0]) columnInfos = settings[0].aoColumns;
      //构造DataTable前的设置
      else if (settings.columns) columnInfos = settings.columns;
    ...
    addDataTableData: function (dataTable, datas, startRowIndex, notDraw) {
      ...
      if (startRowIndex != null) {
        rows.every(function (rowIndex) {
          ...
          if (dataIndex >= datas.length) removeRowIndexes.push(rowIndex);
          else this.data(datas[dataIndex]);
      ...
      if (clickEvent.ctrlKey) {
        if ($clickCell.hasClass("selected")) {
          ...
          indexes = indexes.concat(
              $.makeArray(dataTable.cells(".selected").indexes())
          );
    ...
    handleCellNavigationForKeydown: function (dataTable, keydownEvent) {
      ...
      if (!selectedCells || selectedCells.length == 0) return;
        ...
        if (nextCellIndex != null) {
          ...
          if (nextCellIndex.row > maxRowIndex) nextCellIndex.row = maxRowIndex;
          if (nextCellIndex.row < 0) nextCellIndex.row = 0;

          if (nextCellIndex.column > maxColumnIndex)
            nextCellIndex.column = maxColumnIndex;
          if (nextCellIndex.column < 1) nextCellIndex.column = 1;
    ...
    evalCellIndexesForRange: function (cellIndexes, minColumnThreashold) {
      if (minColumnThreashold == undefined) minColumnThreashold = 0;

      var minRow = Number.MAX_VALUE,
          minColumn = Number.MAX_VALUE,
          maxRow = 0,
          maxColumn = 0;
      ...
      if (minColumn < minColumnThreashold) minColumn = minColumnThreashold;

      if (maxColumn < minColumnThreashold) maxColumn = minColumnThreashold;
    ...
    updateDataTableHeight: function (dataTableElements, height, adjustWidth) {
      if (height == null && adjustWidth == null) return;
      ...
      for (var i = 0; i < dataTableElements.length; i++) {
        ...
        if (adjustWidth) dataTable.columns.adjust();

        if (dataTable.init().fixedColumns) dataTable.fixedColumns().relayout();
      }
    },
     ...
     */
    bindResizeDataTableHandler: function (
        dataTableElements,
        calChangedDataTableHeightFunc
    ) {
      var resizeHandler = function (event) {
        ...
        var toggablePanel = $dataTable0.closest(
            ".ui-tabs-panel, ." + $.TOGGLABLE_TABLE_PANEL_CLASS_NAME
        );
      ...
      var toggablePanel = $(dataTableElements[0]).closest(
          ".ui-tabs-panel, ." + $.TOGGLABLE_TABLE_PANEL_CLASS_NAME
      );
      if (toggablePanel.length > 0) {
        $.bindPanelShowCallback(toggablePanel, function ($panel) {
          if (!$.isResizeDataTableWhenShow($panel)) return;
    ...
    callPanelShowCallback: function ($panel) {
      var callbacks = $panel.data("_SHOW_CALLBACK");
      if (callbacks) {
        for (var i = 0; i < callbacks.length; i++) callbacks[i]($panel);
      }
      ...
      if (subTabs.length > 0) {
        subTabs.each(function () {
          ...
          if (subTabId.charAt(0) == "#") subTabId = subTabId.substr(1);
     ...
     */
    buildSearchConditionAutocompleteSource: function (
        table,
        sqlIdentifierQuote
    ) {
      var source = [];
      ...
      for (var i = 0; i < table.columns.length; i++) {
        ...
        source.push({
          label: column.name,
          value: sqlIdentifierQuote + column.name + sqlIdentifierQuote,
        });
      }
    ...
    fileuploadaddHandlerForUploadInfo: function (e, data, $fileUploadInfo) {
      ...
      $("<div class='file-name' />")
          .html(fileName.length > 13 ? fileName.substr(0, 10) + "..." : fileName)
          .attr("title", fileName)
     ...
     */
    fileuploadprogressallHandlerForUploadInfo: function (
        e,
        data,
        $fileUploadInfo
    ) {
      ...
      if (progress <= 100 && progress > 99) progress = 99;
    ...
    getMapKeyValueArray: function (map, keySortfunction) {
      ...
      for (var key in map) keyArray.push(key);

      if (keySortfunction != undefined) keyArray.sort(keySortfunction);
    ...
    getPropertyCount: function (obj) {
      ...
      for (var p in obj) propertyCount++;
    ...
    deepEquals: function (a, b, ignorePropertyNames, aStack, bStack) {
      ...
      //基本类型
      if (type != "array" && type != "object") return a == b;
      ...
      //对象
        ...
        while (length--) {
          ...
          if ($.meta.containsOrEquals(ignorePropertyNames, key)) continue;

          if (
              !(
                  b.hasOwnProperty(key) &&
                  $.deepEquals(a[key], b[key], null, aStack, bStack)
              )
          )
            return false;
        }
    ...
    compareVersion: function (v0, v1) {
      ...
      for (var i = 0; i < Math.max(vv0.length, vv1.length); i++) {
        if (vv0[i] > vv1[i]) return 1;
        else if (vv0[i] < vv1[i]) return -1;
      }
    ...
    resolveVersion: function (version) {
      ...
      if (bidx > -1) {
        if (bidx + 1 < version.length) ary[3] = version.substring(bidx + 1);
  ...
  $.extend($.dataTableUtil, {
    ...
    removeCheckColumnProperty: function (data) {
      ...
      for (var i = 0; i < datas.length; i++) {
        var ele = datas[i];
        for (var p in ele) {
          if (p == $.dataTableUtil.TABLE_CHECK_COLUMN_NAME) delete ele[p];
        }
      }
    ...
    buildCheckCloumn: function (title) {
      var column = {
        title: title,
        data: $.dataTableUtil.TABLE_CHECK_COLUMN_NAME,
        defaultContent: "",
        width: "3em",
        orderable: false,
    ...
    bindCheckColumnEvent: function (dataTable) {
      ...
      if (settins.fixedColumns) dataTable.fixedColumns().relayout();

      //行选中框
      $(dataTable.table().body())
          .on("click", ".column-check", function (event) {
            event.stopPropagation();
            ...
            if (event.shiftKey) {
              ...
              if (isSelected) $preTr = $tr.prevUntil(":not(.selected)").last();
              else $preTr = $tr.prevAll(".selected:first");
              ...
              if ($preTr.length > 0) {
                ...
                if (isSelected) $nextTr = $tr.nextUntil(":not(.selected)").last();
                else $nextTr = $tr.nextAll(".selected:first");
              ...
              for (var i = rangeStart; i < rangeEnd; i++) selectedIndexes.push(i);

              if (isSelected) dataTable.rows(selectedIndexes).deselect();
              else dataTable.rows(selectedIndexes).select();
            } else {
              if (isSelected) dataTable.row($tr).deselect();
              else {
          ...
          .on("mouseover mouseout", ".column-check", function (event) {
            ...
            $(".dataTable", $tableContainer).each(function () {
              ...
              else $("tr:eq(" + rowIndex + ")", this).removeClass("hover");
            });
          });

      //固定选择列后hover效果默认不能同步，需要自己实现
      $(dataTable.table().body()).on("mouseover mouseout", "tr", function (
          event
      ) {
    ...
    executeOnSelect: function (dataTable, illegalTip, callback) {
      ...
      if (!rowsData || rowsData.length != 1) $.tipInfo(illegalTip);
      else {
    ...
    getRowsData: function (dataTable, rows) {
      if (rows == undefined) rows = dataTable.rows();
    ...
    addRowData: function (dataTable, data) {
      if ($.isArray(data)) dataTable.rows.add(data).draw();
      else dataTable.row.add(data).draw();
    },
    ...
    setRowData: function (dataTable, rowIndex, data) {
      ...
      } else dataTable.row(rowIndex).data(data).draw();
    },

    deleteRow: function (dataTable, rowIndex) {
      if (rowIndex.length != undefined)
        dataTable.rows(rowIndex).remove().draw();
      else dataTable.row(rowIndex).remove().draw();
    },
  ...
  $.extend($.sqlAutocomplete, {
     ...
     */
    resolveAutocompleteInfo: function (
        editor,
        session,
        pos,
        prefix,
      ...
      try {
        ...
        while (true) {
          ...
          autocompleteInfo = $.sqlAutocomplete.resolveTokenAutocompleteInfo(
              tokens,
              token,
              isInToken,
              delimiter,
     ...
     */
    resolveTokenAutocompleteInfo: function (
        tokens,
        token,
        isInToken,
        delimiter,
      ...
      if (tokens.length < 1 || !token) return { type: "none" };

      //注释
      if (
          isInToken &&
          ($.sqlAutocomplete.isTokenComment(token) ||
              $.sqlAutocomplete.isTokenString(token))
      )
        return { type: "none" };
      ...
      if (isInToken) {
        ...
        //如果没有前置Token，则向上级查找
        if (!prevToken) prevToken = token.parent;
      }

      var prevToken = $.sqlAutocomplete.findToken(
          prevToken,
          false,
          $.sqlAutocomplete.isNotTokenComment,
          false
      );

      if (!prevToken) return { type: "prepend" };

      var isTokenDelimiter = function (token) {
        return (
            $.sqlAutocomplete.isTokenTextValue(prevToken, delimiter) ||
            $.sqlAutocomplete.isTokenTextValue(prevToken, ";")
        );
      };

      //SQL分隔符
      if (isTokenDelimiter(prevToken)) return { type: "none" };
      ...
      //表
      if (
          $.sqlAutocomplete.isTokenKeyword(
              prevToken,
              $.sqlAutocomplete.keywordsNextIsTable
          )
      )
        return { type: "table" };
      ...
      if (
          $.sqlAutocomplete.isTokenKeyword(
              prevToken,
              $.sqlAutocomplete.keywordsNextIsColumn
          )
      ) {
        var tableTokenPredicate = function (token) {
          if (!$.sqlAutocomplete.isTokenIndentifier(token)) return false;

          if (isTokenDelimiter(token)) return false;

          if (!tableAlias) return true;

          if (
              $.sqlAutocomplete.isTokenTextValue(token, tableAlias) ||
        ...
        if ($.sqlAutocomplete.isTokenKeyword(prevToken, "SELECT")) {
          var fromToken = $.sqlAutocomplete.findToken(
              prevToken,
              true,
              function (token) {
                return (
                    $.sqlAutocomplete.isTokenKeyword(token, "FROM") ||
          ...
          if (isTokenDelimiter(fromToken)) return { type: "none" };

          var tableToken = $.sqlAutocomplete.findToken(
              fromToken,
              true,
              tableTokenPredicate,
              true
          ...
          if (tableToken) {
            if (isTokenDelimiter(tableToken)) return { type: "none" };
            else return { type: "column", table: tableToken.value };
          } else return { type: "append" };
        } else if (
            $.sqlAutocomplete.isTokenKeyword(prevToken, "WHERE") ||
          ...
          } else return { type: "prepend" };
        } else if ($.sqlAutocomplete.isTokenKeyword(prevToken, "SET")) {
          var updateToken = $.sqlAutocomplete.findToken(
              prevToken,
              false,
              function (token) {
                return (
          ...
          if (isTokenDelimiter(updateToken)) return { type: "none" };
      ...
      //列 INTO [table] (...)
          ...
          $.sqlAutocomplete.isTokenIndentifier(prevToken) &&
          $.sqlAutocomplete.isTokenKeyword(prevToken.prev, "INTO")
      ) {
        return { type: "column", table: prevToken.value };
      } else
        return $.sqlAutocomplete.resolveTokenAutocompleteInfo(
            tokens,
            prevToken,
            true,
            delimiter,
    ...
    isKeyword: function (text, keywords) {
      if (!keywords) keywords = $.sqlAutocomplete.keywords;

      if (!text) return false;

      if ($.sqlAutocomplete.maxKeywordLength == null)
        $.sqlAutocomplete.maxKeywordLength = $.sqlAutocomplete.getMaxKeywordLength(
            $.sqlAutocomplete.keywords
        );

      if (text.length > $.sqlAutocomplete.maxKeywordLength) return false;

      if (typeof keywords == "string") return keywords == text.toUpperCase();
      else return keywords[text.toUpperCase()];
    },
    ...
    findToken: function (token, forward, predicate, onlySibling) {
      if (onlySibling == undefined) onlySibling = false;
    ...
    findTokenBySqlIndex: function (tokens, sqlIndex) {
      ...
      for (var i = 0; i < tokens.length; i++) {
        ...
        //在token起始位置
        if (token.startIndex == sqlIndex) return token;
        //在token中
        else if (
            token.startIndex < sqlIndex &&
            (token.endIndex == null || sqlIndex < token.endIndex)
        ) {
          if ($.sqlAutocomplete.isTokenBracketBlockWithValue(token)) {
            var subToken = $.sqlAutocomplete.findTokenBySqlIndex(
                token.value,
                sqlIndex
            );
        ...
        //在token和token.next的中间
            ...
            token.endIndex <= sqlIndex &&
            token.next &&
            token.next.startIndex > sqlIndex
        ) {
          return token;
        }
      }
    ...
    isTokenTextValue: function (token, textValue) {
      ...
      if (textValue == undefined) return true;

      if (token.value.length != textValue.length) return false;

      var upperValue = token.value.toUpperCase();

      if (upperValue == textValue) return true;

      return upperValue == textValue.toUpperCase();
    },

    isTokenString: function (token) {
      return $.sqlAutocomplete.isTokenType(
          token,
          $.sqlAutocomplete.TOKEN_STRING
      );
    },
    ...
    isTokenIndentifier: function (token) {
      return $.sqlAutocomplete.isTokenType(
          token,
          $.sqlAutocomplete.TOKEN_IDENTIFIER
      );
    },
    ...
    isTokenComment: function (token) {
      return (
          $.sqlAutocomplete.isTokenType(
              token,
              $.sqlAutocomplete.TOKEN_COMMENT_LINE
          ) ||
          $.sqlAutocomplete.isTokenType(
              token,
              $.sqlAutocomplete.TOKEN_COMMENT_BLOCK
          )
      );
    },
    ...
    isTokenBracketBlockWithValue: function (token) {
      if (!$.sqlAutocomplete.isTokenBracketBlock(token)) return false;
    ...
    isTokenBracketBlock: function (token) {
      return $.sqlAutocomplete.isTokenType(
          token,
          $.sqlAutocomplete.TOKEN_BRACKET_BLOCK
      );
    },
    ...
    isTokenKeyword: function (token, keywords) {
      if (
          !$.sqlAutocomplete.isTokenType(token, $.sqlAutocomplete.TOKEN_KEYWORD)
      )
        return false;
    ...
    resolveTokens: function (sql, tokens, startIndex) {
      ...
      for (var i = startIndex; i < sql.length; ) {
        ...
        if (token.startIndex == token.endIndex) return token.endIndex;
    ...
    resolveToken: function (sql, startIndex, token) {
      if (token) {
        if (token.startIndex == null) throw new Error("illegal");
      ...
      for (var i = startIndex; i < sql.length; ) {
        ...
        //标点符号
          ...
          token = {
            type: $.sqlAutocomplete.TOKEN_PUNCTUATION,
            startIndex: i,
            endIndex: i + 1,
            value: c,
          };
    ...
    resolveTokenString: function (sql, startIndex, token) {
      ...
      //没有结束符
      if (chars.length > 0) token.value = token.value + chars.join("");
    },
    ...
    resolveTokenBracketBlock: function (sql, startIndex, token) {
      ...
      if (token.value && token.value.length > 0) {
        ...
        if (
            tailToken.endIndex != null &&
            tailToken.endIndex < sql.length &&
            sql.charAt(tailToken.endIndex) == ")"
        )
      ...
      token.value = [];
      var endIndex = $.sqlAutocomplete.resolveTokens(
          sql,
          token.value,
          startIndex
      );
  ...
  $.handleAjaxOperationMessage = function (jqXHR, errorThrown) {
    if (!window._showAjaxOperationMessageDetail) {
      window._showAjaxOperationMessageDetail = function () {
        ...
        var isSuccessMessage =
            "true" == $operationMessageParent.attr("success");

        var $dialog = $(
            "<div id='dialog-" +
            new Date().getTime() +
            "' class='operation-message-dialog'></div>"
        ).appendTo(document.body);

        var $messageDetail = $("<div class='message-detail' />");
        if (!isSuccessMessage) $messageDetail.addClass("ui-state-error");
        $messageDetail.appendTo($dialog);
        $messageDetail.html(
            $(".message-detail", $operationMessageParent).html()
        );
        ...
        $._dialog($dialog, {
          ...
          classes: {
            "ui-dialog":
                ...
                (isSuccessMessage ? "" : "ui-state-error"),
          },
        });

        var $dialogWidget = $dialog.dialog("widget");

        $(".ui-dialog-title", $dialogWidget).prepend(
            "<span class='ui-icon " +
            (isSuccessMessage ? "ui-icon-circle-check" : "ui-icon-alert") +
            "'></span>"
        );
        ...
        if (!isSuccessMessage) {
          ...
          $(".ui-dialog-titlebar-close", $dialogWidget).addClass(
              "ui-state-error"
          );
        }
      };
    ...
    if (jqXHR.responseText) {
      ...
      if ($operationMessageParent.length == 0)
        $operationMessageParent = $(
            "<div id='__operationMessageParent' style='display:none;' />"
        ).appendTo(document.body);
      ...
      if (
        ...
        ).appendTo($operationMessageParent);
        var $mdiv = $("<div class='message' />")
            .appendTo($omdiv)
            .html(operationMessage.message);
        ...
        if (operationMessage.detail) {
          ...
          else $("<div />").appendTo($ddiv).html(operationMessage.detail);
        }

        hasMessage = true;
      } else {
        if (isSuccessMessage) hasMessage = false;
        else {
          //操作消息的HTML响应
      ...
      if (hasMessage) {
        ...
        if ($(".message-detail", $operationMessageParent).length > 0)
          message +=
              "<span class='ui-icon ui-icon-comment message-detail-icon' onclick='_showAjaxOperationMessageDetail();'></span>";

        if (isSuccessMessage) $.tipSuccess(message);
        else $.tipError(message);
      }
    } else {
      if (isSuccessMessage);
      else {
  ...
  $.ajaxPrefilter(function (options, originalOptions, jqXHR) {
    // 所有请求追加自定义权限请求头 Authorization
    var access_token;
    // var access_token = "04c06872-c4f1-41a6-84aa-36842a504798";
    if (sessionStorage.getItem("tax.back.authData")) {
      access_token = JSON.parse(sessionStorage.getItem("tax.back.authData")).val
          .access_token;
    }
    jqXHR.setRequestHeader("Authorization", "Bearer " + access_token);

    if (originalOptions.contentType != $.CONTENT_TYPE_JSON) return;
