...
public class SchemaController extends AbstractSchemaConnTableController {
    ...
    public ResponseEntity<OperationMessage> saveAdd(HttpServletRequest request, HttpServletResponse response,
        ...
        int i = someMapper.querySchemaNameNum(schema.getTitle(), user.getId());
        if (i > 0) {
            return buildOperationMessageFailResponseEntity(request,HttpStatus.BAD_REQUEST,"新增失败，存在同名数据源","新增失败，存在同名数据源");
        }
    ...
    public ResponseEntity<OperationMessage> saveEdit(HttpServletRequest request, HttpServletResponse response,
        ...
        //todo 切换用户体系
        User user = WebUtils.getUser(request, response);
        int i = someMapper.querySchemaNameById(schema.getTitle(), user.getId(), schema.getId());
        if(i>0){
            return buildOperationMessageFailResponseEntity(request,HttpStatus.BAD_REQUEST,"编辑失败，存在同名数据源","编辑失败，存在同名数据源");
        }
...
//        if (null == user||"55650a4dd0454f14920a5b8b93002a7e".equals(user.getId())) {
